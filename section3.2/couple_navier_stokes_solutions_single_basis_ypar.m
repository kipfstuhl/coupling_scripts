function [blocks,blocks_t] = couple_navier_stokes_solutions_single_basis_ypar(fespaces_u,fespaces_p,bcs_flags,bf,gausspoints)
...
matbf = @(x) [bf(x) bf(x);bf(x) bf(x)];

for j = 1:ncouplings    
    bc_flags_aux = [0 bcs_flags(2,j) 0 bcs_flags(4,j)]';
