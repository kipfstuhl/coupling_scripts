...
if (set == 1)
    ...
    mu = 1;
    nu = mu;
    fun = @(x) [-mu*(u1exdxdx(x)+u1exdydy(x)) +  pexdx(x);
...
stresstens = @(x) graduex(x) - [pex(x) 0; 0 pex(x)];
...
%         
    ...
    [A1,rhs1] = assembler_steady_stokes(fespace1_u,fespace1_p,fun,nu,dirichlet_functions,neumann_functions);
    [A2,rhs2] = assembler_steady_stokes(fespace2_u,fespace2_p,fun,nu,dirichlet_functions,neumann_functions);
    [A3,rhs3] = assembler_steady_stokes(fespace3_u,fespace3_p,fun,nu,dirichlet_functions,neumann_functions);
    ...
    %     bf = @(x) 1;
    %     [B1,B1_t] = couple_navier_stokes_solutions_single_basis_new(fespaces_u,fespaces_p,bcs_flags,bf,gausspoints);
    %     B = [B;B1];
    %     B_t = [B_t B1_t];
    %
    for n_itx = 0:7
    ...
    lagmul = 0*x;
    for i = 1:n_itx+1
        lagmul = lagmul - bfs1{i}([x;yline*x.^0])*solm(2*i);
    end
    ...
    hold on
    % plot exact stress
    plot(x,u1exdx([0.5*x.^0;x])-pex([xline*x.^0;x]),'--k');
