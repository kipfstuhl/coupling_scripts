...
%% We first solve a Stokes problem to get the intial conditions
...
sol4 = sol(indices4);
sol5 = sol(indices5);
sollm = sol(n1+n2+n3+n4+n5+1:end);
...
B1 = [-blocks_interface1{1} blocks_interface1{2} blocks_interface1{3} sparse(nlagmul1,n4) blocks_interface1{4}];
B2 = [sparse(nlagmul2,n1) sparse(nlagmul2,n2) sparse(nlagmul3,n3) blocks_interface2{1} -blocks_interface2{2}];
B3 = [sparse(nlagmul3,n1) sparse(nlagmul2,n2) blocks_interface3{1} -blocks_interface3{2} -blocks_interface3{3}];
B4 = [sparse(nlagmul4,n1) blocks_interface4{1} -blocks_interface4{2} sparse(nlagmul4,n4) sparse(nlagmul4,n5)];
...
jac = @(u) [blkdiag(build_jac_navier_stokes(A1,u(indices1),fespace1_u), ...
            ...
            build_jac_navier_stokes(A5,u(indices5),fespace5_u)) B';
            B zerosp];
tol = 1e-7;
maxit = 100;

sol = solve_with_newtons_method(f,x0,jac,tol,maxit);
...
%% Visualize and export the solution
...
finemesh = create_mesh(0,0,1,1,200,200);
fespace_ufine = create_fespace(finemesh,'P1',[1 1 1 1]);
fespace_pfine = create_fespace(finemesh,'P1',[1 1 1 1]);

solutions = {solstr1, solstr2, solstr3, solstr4, solstr5};

finesol = interpolate_multiple_solutions(solutions,fespace_ufine,fespace_pfine);
export_vtk_fluid(finesol,'finesol');
