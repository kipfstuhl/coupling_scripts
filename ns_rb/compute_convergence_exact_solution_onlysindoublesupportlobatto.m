...
for freqq = 0:2:10
    ...
    for ref = [1 3 5 7 9]
        ...
        cur = 6;
        for i = 1:3 % interface
            mylang_n = lagrange_mult(cur+1:2:cur+nfreqs(i)*2);
            mylang_t = lagrange_mult(cur+2:2:cur+nfreqs(i)*2);
            ...
            for j = 1:length(mylang_n)
                st_n = st_n + sin(j*x'/ls(i)*pi/2) * mylang_n(j);
                st_t = st_t + sin(j*x'/ls(i)*pi/2) * mylang_t(j);
            end
...
save('erroronlysindoublesupportlobatto.mat','totalerr');
save('erroronlysindoublesupportlobatto_lag.mat','totalerr_lag');
