...
%         draw_mesh(mesh_out2,[0 0 0]);
        ...
        cur = 0;
        for i = 1:3 % interface
            mylang_n = lagrange_mult(cur+1:2:cur+nfreqs(i)*2+1);
            mylang_t = lagrange_mult(cur+2:2:cur+nfreqs(i)*2+2);
            ...
            st_n = ones(npoints,1) * mylang_n(1);
            st_t = ones(npoints,1) * mylang_t(1);
            
            for j = 2:length(mylang_n)
                st_n = st_n + sin((j-1)*x'/ls(i)*pi/2) * mylang_n(j);
                st_t = st_t + sin((j-1)*x'/ls(i)*pi/2) * mylang_t(j);
            end
...
save('erroronlysinsinglesupport.mat','totalerr')
save('erroronlysinsinglesupport_lag.mat','totalerr_lag');
