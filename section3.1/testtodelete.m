...
n1x = N/2;
n2x = N/2;
n1y = floor(N*2); % this is to get a non conformal mesh
n2y = N;
...
for i = 1:nmodes
    ...
    v1 = apply_neumann_bc(v1,fespace1,@(x) [0;sin(x(2)*pi*i);0;0]);
    v2 = apply_neumann_bc(v2,fespace2,@(x) [0;0;0;sin(x(2)*pi*i)]);
    v11 = apply_neumann_bc(v1,fespace1,@(x) [0;cos(x(2)*pi*i);0;0]);
    v22 = apply_neumann_bc(v2,fespace2,@(x) [0;0;0;cos(x(2)*pi*i)]);
...
%     err = compute_L2_error(fespace,abs(totalsol-interpsol),@(x) 0);
...
save('errors_convergence_wrt_h_non_conformal.mat','errs')