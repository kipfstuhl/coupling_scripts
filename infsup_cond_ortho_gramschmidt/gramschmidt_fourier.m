function bfs = gramschmidt_fourier(freqs)

l2norm = @(f) integral(@(x) f(x).*f(x),0,1);
proj   = @(f1,f2) integral(@(x) f1(x).*f2(x),0,1)/l2norm(f2);

bfs = {};
bfs_nonortho = {};

bfs_nonortho{end+1} = @(x) x.^0;
for i = 1:freqs
    bfs_nonortho{end+1} = @(x) sin(x*pi*i);
    bfs_nonortho{end+1} = @(x) cos(x*pi*i);
end

N = size(bfs_nonortho,2);

bfs = {};
for i = 1:N
    bfs{i} = bfs_nonortho{i};
    for j = 1:i-1
        bfs{i} = @(x) bfs{i}(x) - proj(bfs{i},bfs{j})*bfs{j}(x);
    end
    bfs{i} = @(x) bfs{i}(x)/l2norm(bfs{i});
end
clear all
close all
clc
